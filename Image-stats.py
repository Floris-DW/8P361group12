...
for key,value in paths.items():
    ...
    tot_og = len(L_images)
    if key == 'Train':
        tot = int(tot_og*train_split)
    elif key == 'Valdation':
        tot = int(tot_og*(1-train_split))
    else:
        tot = tot_og

    mean = 0 # is an numpy image array
    for i,p in enumerate(L_images):
        if key == 'Train' and i > train_split*tot_og:
            break # only check the first train_split% images
        if key == 'Valdation' and i <= train_split*tot_og:
            continue # skip the first train_split% images
        im = PIL.Image.open(value+'/'+p)
        im = np.asarray(im)/255
        mean += im
        print(f'\r[progres - mean] {i+1 -(key == "Valdation")*(tot_og-tot) }/{tot}',end="")
    mean = mean/tot
    ...
    for i,p in enumerate(L_images):
        ...
        im = np.asarray(im)/255
        Ex_m += (im-mean)**2
        print(f'\r[progres - Ei(xi-mean)^2] {i+1 -(key == "Valdation")*(tot_og-tot) }/{tot}',end="")
    variance =       Ex_m/tot
